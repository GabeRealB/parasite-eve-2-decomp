...
/// The record a cutscene task spawned from `D_mist_parking_801869B8` receives
/// as `Task::spawnArg2`: `field_0` is the save-data view the scene runs under
/// (negative only restores it), `field_1` the caption slot, `field_2` skips
/// straight to the abort path, `field_3` the caption file to load, and
/// `field_4` / `field_8` / `field_C` the sound events for the start, the end
/// and the scene. `field_10` is the scene sub-task's spawn argument and
/// `field_14` / `field_16` the fade lengths.
typedef struct MistParkingSceneRec {
    /* 0x00 */ s8  field_0;
    /* 0x01 */ s8  field_1;
    /* 0x02 */ s8  field_2;
    /* 0x03 */ s8  field_3;
...
/// Fill the play-data panels' weapon and PE usage rows.
