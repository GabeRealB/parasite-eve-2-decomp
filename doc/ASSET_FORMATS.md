...
writing to `asm/USA/incbin/` — a per-worktree build input regenerated by every
split, not a catalogue asset. Both trees are gitignored, so game data stays out of git
either way.
