...
|---|---|---|---|
| `$t2`/`$t4` | `r428` = `%hi(gGpuPrimCursor)` 13 refs/426, `r531` = `&gDisplayState` 11/414, both cross 2 calls | both are **caller-save** allocations (all of `$s0-$s7`,`$fp` are taken); `3·13/426 = 0.092 > 3·11/414 = 0.080` places `r428` first | the original ranked the `gDisplayState` address above the `%hi`: two more weighted mentions of the former, or one fewer `gGpuPrimCursor` access through the same `%hi` pseudo, cross the threshold |
