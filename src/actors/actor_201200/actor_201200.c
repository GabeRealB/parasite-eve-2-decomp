...
s32 func_actor_201200_80149F50(GsCOORDINATE2* coord, GpRec18* recs, s16 count, SVECTOR* push)
{
    u8*                      head;
    ...
    *(u8**)G_SCRATCH_HEAD = head - sizeof(Actor104000AvoidScratch);
    ...
    for (s->i = 0; s->i < count; s->i++) {
        ...
        if (ABS(s->dir.vz) < 0x818) {
            s->angle[s->count] = Actor201200_BearingXZ((SVECTOR3*)&recs[s->i].field_8, &s->eye);
        } else {
            s->angle[s->count] = Actor201200_BearingXY((SVECTOR3*)&recs[s->i].field_8, &s->eye);
        }
        s->ok[s->count] = 1;
        s->count++;
        if (s->count >= 8) {
