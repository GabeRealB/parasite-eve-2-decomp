...
static __inline__ s32 Actor323300_AccumulateRotation(GsCOORDINATE2* joint, MATRIX* out, GsCOORDINATE2* stop)
{
    MATRIX         matrix;
    GsCOORDINATE2* coord;
...
static __inline__ GsCOORDINATE2* Actor323300_LocalizeRotation(GsCOORDINATE2* joint, MATRIX* rotation)
{
    MATRIX         matrix;
    MATRIX         normal;
    MATRIX         transposed;
...
void func_actor_323300_80163188(GsCOORDINATE2* coord, s16 angle)
{
    ...
    *(MATRIX**)G_SCRATCH_HEAD -= 1;
    ...
    Actor323300_AccumulateRotation(coord, rotation, &gGfxViewCoord);
    func_8004BFF8(angle, rotation);
    RotMatrixX(angle / 2, rotation);
    out = Actor323300_LocalizeRotation(coord, rotation);
