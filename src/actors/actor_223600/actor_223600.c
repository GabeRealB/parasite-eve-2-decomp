...
/// 0x54-byte block `func_actor_223600_8014A4B8` takes from the scratchpad head
...
typedef struct Actor223600AvoidScratch {
    /* 0x00 */ MATRIX   m;
    /* 0x20 */ SVECTOR  dir;
    /* 0x28 */ SVECTOR3 eye;
...
{
    u8*                    head;
    Actor223600AvoidDelta* d;

    head                  = *(u8**)G_SCRATCH_HEAD;
    d                     = (Actor223600AvoidDelta*)(head - 0x10);
    d->vx                 = p->vx - eye->vx;
...
s32 func_actor_223600_8014A4B8(GsCOORDINATE2* coord, GpRec18* recs, s16 count, SVECTOR* pos)
{
    ...
    head                  = *(u8**)G_SCRATCH_HEAD;
    *(u8**)G_SCRATCH_HEAD = head - sizeof(Actor223600AvoidScratch);
    s                     = (Actor223600AvoidScratch*)*(u8**)G_SCRATCH_HEAD;
    s->blocked            = 0;
    pos->vz               = 0;
    pos->vy               = 0;
    pos->vx               = 0;
    ...
    *(u8**)G_SCRATCH_HEAD = (u8*)*(u8**)G_SCRATCH_HEAD + sizeof(Actor223600AvoidScratch);
    return s->blocked != 0;
}
