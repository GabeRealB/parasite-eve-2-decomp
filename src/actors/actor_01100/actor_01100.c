...
/// 0x88-byte block taken from the scratchpad head by `Actor01100_Fn000E8`.
/// `pos` is the coordinate's world translation, `offset` the latest push-out
/// (scaled down to length 0x100 at the end), `last` its XZ copy, `i` the record
/// cursor and `hit` the return value; `dist` takes 0x7FFE at the record that
/// ends the table.
typedef struct Actor01100RepelScratch {
    ...
    /* 0x82 */ byte    pad_82[4];
    /* 0x86 */ s16     hit;
} Actor01100RepelScratch;
STATIC_ASSERT_SIZEOF(Actor01100RepelScratch, 0x88);
...
static __inline__ void Actor01100_CalcPush(SVECTOR* pos, GpRec18* rec, SVECTOR* out)
{
    VECTOR d;
    VECTOR n;
    s32    t;
...
{
    Actor01100RepelScratch* head;
    Actor01100RepelScratch* s;
    Actor01100RepelScratch* blk;
    SVECTOR*                offset;
    ...
    coord->flg                                = 0;
    head                                      = *(Actor01100RepelScratch**)G_SCRATCH_HEAD;
    blk                                       = head - 1;
    *(Actor01100RepelScratch**)G_SCRATCH_HEAD = blk;
    s                                         = blk;
    ...
    for (s->i = 0; s->i < count; s->i++) {
        ...
        if (s->kind == 0x10000 || s->kind == 0x30000) {
            s->hit = 1;
            Actor01100_CalcPush(&s->pos, &recs[s->i], &s->offset);
    ...
    coord->flg                                 = 0;
    *(Actor01100RepelScratch**)G_SCRATCH_HEAD += 1;
    return s->hit;
}

/// 0x54-byte block taken from the scratchpad head by `Actor01100_Fn00430`.
/// `angle` / `ok` hold up to eight bearings collected from the records, `i` /
/// `j` are the loop cursors, and `blocked` is set when a record's kind is
/// 0x10000.
typedef struct Actor01100AvoidScratch {
    /* 0x00 */ MATRIX   m;
    /* 0x20 */ SVECTOR  dir;
    /* 0x28 */ SVECTOR3 eye;
    ...
    /* 0x52 */ u8       count;
    /* 0x53 */ u8       blocked;
} Actor01100AvoidScratch;
STATIC_ASSERT_SIZEOF(Actor01100AvoidScratch, 0x54);

/// 0x10-byte block the bearing helpers take from the scratchpad head, below
/// `Actor01100AvoidScratch`: an obstacle's offset from the eye, widened to
/// words.
typedef struct Actor01100AvoidDelta {
    /* 0x0 */ s32  vx;
    /* 0x4 */ s32  vy;
    /* 0x8 */ s32  vz;
    /* 0xC */ byte pad_C[0x4];
} Actor01100AvoidDelta;
STATIC_ASSERT_SIZEOF(Actor01100AvoidDelta, 0x10);

/// Bearing of `p` from `eye` in the XZ plane.
static __inline__ s16 Actor01100_BearingXZ(SVECTOR3* p, SVECTOR3* eye)
{
    u8*                   head;
    Actor01100AvoidDelta* d;

    head                  = *(u8**)G_SCRATCH_HEAD;
    d                     = (Actor01100AvoidDelta*)(head - 0x10);
    d->vx                 = p->vx - eye->vx;
...
/// Bearing of `p` from `eye` in the XY plane.
static __inline__ s16 Actor01100_BearingXY(SVECTOR3* p, SVECTOR3* eye)
{
    u8*                   head;
    Actor01100AvoidDelta* d;
...
s32 Actor01100_Fn00430(GsCOORDINATE2* coord, GpRec18* recs, s16 count, SVECTOR* pos)
{
    ...
    head                  = *(u8**)G_SCRATCH_HEAD;
    *(u8**)G_SCRATCH_HEAD = head - sizeof(Actor01100AvoidScratch);
    s                     = (Actor01100AvoidScratch*)*(u8**)G_SCRATCH_HEAD;
    s->blocked            = 0;
    pos->vz               = 0;
    pos->vy               = 0;
    pos->vx               = 0;
    ...
    for (s->i = 0; s->i < count; s->i++) {
        ...
        if (ABS(s->dir.vz) < 0x818) {
            s->angle[s->count] = Actor01100_BearingXZ((SVECTOR3*)&recs[s->i].point, &s->eye);
        } else {
            s->angle[s->count] = Actor01100_BearingXY((SVECTOR3*)&recs[s->i].point, &s->eye);
        }
        s->ok[s->count] = 1;
        s->count++;
        if (s->count >= 8) {
    ...
    *(u8**)G_SCRATCH_HEAD = (u8*)*(u8**)G_SCRATCH_HEAD + sizeof(Actor01100AvoidScratch);
    return s->blocked != 0;
}
