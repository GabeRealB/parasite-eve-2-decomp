...
/// 0x88-byte `G_SCRATCH_HEAD` block `func_actor_323000_80162198` takes while it
...
/// record.
typedef struct Actor323000RepelScratch {
    ...
    /* 0x82 */ byte    pad_82[4];
    /* 0x86 */ s16     hit;
} Actor323000RepelScratch;
STATIC_ASSERT_SIZEOF(Actor323000RepelScratch, 0x88);

/// 0x54-byte `G_SCRATCH_HEAD` block `func_actor_323000_801624E0` takes while it
...
typedef struct Actor323000AvoidScratch {
    /* 0x00 */ MATRIX   m;
    /* 0x20 */ SVECTOR  dir;
    /* 0x28 */ SVECTOR3 eye;
    ...
    /* 0x52 */ u8       count;
    /* 0x53 */ u8       blocked;
} Actor323000AvoidScratch;
STATIC_ASSERT_SIZEOF(Actor323000AvoidScratch, 0x54);

/// 0x10-byte block the bearing helpers of `func_actor_323000_801624E0` carve
/// below the scratch head: an obstacle's offset from the eye, widened to words.
typedef struct Actor323000AvoidDelta {
    /* 0x0 */ s32  vx;
    /* 0x4 */ s32  vy;
    /* 0x8 */ s32  vz;
    /* 0xC */ byte pad_C[0x4];
} Actor323000AvoidDelta;
STATIC_ASSERT_SIZEOF(Actor323000AvoidDelta, 0x10);
...
static __inline__ s32 Actor323000_AccumulateRotation(GsCOORDINATE2* joint, MATRIX* out, GsCOORDINATE2* stop)
{
    MATRIX         matrix;
    GsCOORDINATE2* coord;
...
static __inline__ GsCOORDINATE2* Actor323000_LocalizeRotation(GsCOORDINATE2* joint, MATRIX* rotation)
{
    MATRIX         matrix;
    MATRIX         normal;
    MATRIX         transposed;
...
void func_actor_323000_80161E8C(GsCOORDINATE2* coord, s16 yaw)
{
    ...
    *(MATRIX**)G_SCRATCH_HEAD -= 1;
    ...
    Actor323000_AccumulateRotation(coord, rotation, &gGfxViewCoord);
    func_8004BFF8(yaw, rotation);
    out = Actor323000_LocalizeRotation(coord, rotation);
...
static __inline__ void Actor323000_CalcPush(SVECTOR* pos, GpRec18* rec, SVECTOR* out)
{
    VECTOR d;
    VECTOR n;
    s32    t;
...
{
    Actor323000RepelScratch* head;
    Actor323000RepelScratch* s;
    Actor323000RepelScratch* blk;
    SVECTOR*                 offset;
    ...
    coord->flg                                 = 0;
    head                                       = *(Actor323000RepelScratch**)G_SCRATCH_HEAD;
    blk                                        = head - 1;
    *(Actor323000RepelScratch**)G_SCRATCH_HEAD = blk;
    s                                          = blk;
    ...
    for (s->i = 0; s->i < count; s->i++) {
        ...
        if (s->kind == 0x10000 || s->kind == 0x30000) {
            s->hit = 1;
            Actor323000_CalcPush(&s->pos, &recs[s->i], &s->offset);
    ...
    coord->flg                                  = 0;
    *(Actor323000RepelScratch**)G_SCRATCH_HEAD += 1;
    return s->hit;
}

/// Bearing of `p` from `eye` in the XZ plane, staged in a scratch block of its
/// own that is released before `ratan2` runs.
static __inline__ s16 Actor323000_BearingXZ(SVECTOR3* p, SVECTOR3* eye)
{
    u8*                    head;
    Actor323000AvoidDelta* d;

    head                  = *(u8**)G_SCRATCH_HEAD;
    d                     = (Actor323000AvoidDelta*)(head - 0x10);
    d->vx                 = p->vx - eye->vx;
...
/// close to vertical.
static __inline__ s16 Actor323000_BearingXY(SVECTOR3* p, SVECTOR3* eye)
{
    u8*                    head;
    Actor323000AvoidDelta* d;
...
s32 func_actor_323000_801624E0(GsCOORDINATE2* coord, GpRec18* recs, s16 count, SVECTOR* pos)
{
    ...
    head                  = *(u8**)G_SCRATCH_HEAD;
    *(u8**)G_SCRATCH_HEAD = head - sizeof(Actor323000AvoidScratch);
    s                     = (Actor323000AvoidScratch*)*(u8**)G_SCRATCH_HEAD;
    s->blocked            = 0;
    pos->vz               = 0;
    pos->vy               = 0;
    pos->vx               = 0;
    ...
    for (s->i = 0; s->i < count; s->i++) {
        ...
        if (ABS(s->dir.vz) < 0x818) {
            s->angle[s->count] = Actor323000_BearingXZ((SVECTOR3*)&recs[s->i].point, &s->eye);
        } else {
            s->angle[s->count] = Actor323000_BearingXY((SVECTOR3*)&recs[s->i].point, &s->eye);
        }
        s->ok[s->count] = 1;
        s->count++;
        if (s->count >= 8) {
    ...
    *(u8**)G_SCRATCH_HEAD = (u8*)*(u8**)G_SCRATCH_HEAD + sizeof(Actor323000AvoidScratch);
    return s->blocked != 0;
}
