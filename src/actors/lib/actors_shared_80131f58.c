#include "common.h"
#include "actors/actors_shared_80131f58.h"
...
static __inline__ s16 ActorsShared80131f58_BearingXZ(SVECTOR3* p, SVECTOR3* eye)
{
    u8*                    head;
...
/// close to vertical.
static __inline__ s16 ActorsShared80131f58_BearingXY(SVECTOR3* p, SVECTOR3* eye)
{
    u8*                    head;
    Actor104000AvoidDelta* d;
...
s32 ActorsShared80131f58(GsCOORDINATE2* coord, GpRec18* recs, s16 count, SVECTOR* push)
{
    ...
    if (session->viewReady == 1 || D_80072729 == 1) {
        return 0;
    }
    ...
    *(u8**)G_SCRATCH_HEAD = head - sizeof(Actor104000AvoidScratch);
    ...
    for (s->i = 0; s->i < count; s->i++) {
        ...
        if (ABS(s->dir.vz) < 0x818) {
            s->angle[s->count] =
                ActorsShared80131f58_BearingXZ((SVECTOR3*)&recs[s->i].point, &s->eye);
        } else {
            s->angle[s->count] =
                ActorsShared80131f58_BearingXY((SVECTOR3*)&recs[s->i].point, &s->eye);
        }
        s->ok[s->count] = 1;
        s->count++;
        if (s->count >= 8) {
