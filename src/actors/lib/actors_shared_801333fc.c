...
void ActorsShared801333fc(Actor101500* actor)
{
    ...
    blk                       = head - 1;
    head[-1].vx               = coord->coord.t[0] - Wip_SysConfig.field_4->t[0];
    blk->vy                   = 0;
    blk->vz                   = coord->coord.t[2] - Wip_SysConfig.field_4->t[2];
    *(VECTOR**)G_SCRATCH_HEAD = blk;
    work->field_372           = ratan2((s16)head[-1].vx, (s16)blk->vz) & 0xFFF;
    if (coord->coord.t[1] > Wip_SysConfig.field_4->t[1] + 500 ||
        coord->coord.t[1] < Wip_SysConfig.field_4->t[1] - 1800 ||
        ++work->field_362 > 1800) {
        work->field_378 = 1;
    }
