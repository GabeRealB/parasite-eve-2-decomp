...
void func_actor_141000_80131E94(Actor141000* arg0, Actor141000Point* arg1, s32 arg2)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        addPrim((u32*)((((u32)(arg2 << Display_State.field_128) >> 2) & 0xFFC) + (u32)Gpu_CurrentOt) - 20, poly);
        tpage          = Gpu_PrimCursor;
        Gpu_PrimCursor = tpage + 1;
        setlen(tpage, 1);
        tpage->code[0] = 0xE1000425;
        addPrim((u32*)((((u32)(arg2 << Display_State.field_128) >> 2) & 0xFFC) + (u32)Gpu_CurrentOt) - 20, tpage);
        quad += 4;
    }
    for (i = 6; i < 16; i++) {
        a              = quad[0];
