...
/// of exactly three quantities with a hand-rolled sort that undoes its own
/// first exchange, so the quantities are placed in birth order and the address
/// would take `$v0` ahead of the value it feeds. With `$v0` live over the value
/// the address lands in `$v1` instead, as the original does.
s32 func_actor_104900_8013852C(Task* task, s32 arg1, s32 flags)
{
    ...
    mode  = flags ^ 1;
    work  = (Actor104900Work*)task->idMap;
    ...
    if (work->field_BA0 != mode) {
        ...
        if ((mode << 0x18) == 0) {
            ...
            for (i = 0; i < 4; i++) {
                obj        = (GpObj*)((u8*)work + (OFFSET_OF(Actor104900Work, field_9A8) + i * 0x20));
                value      = obj->flags;
                value     &= 0x3FFF;
                obj->flags = (u16)value;
            }
