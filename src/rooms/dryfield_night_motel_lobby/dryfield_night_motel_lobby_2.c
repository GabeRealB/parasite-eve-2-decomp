...
typedef struct {
    ...
    /* 0x14 */ s16 field_14;
    /* 0x16 */ s16 field_16;
} DryfieldNightMotelLobbyCapScript;
STATIC_ASSERT_SIZEOF(DryfieldNightMotelLobbyCapScript, 0x18);
...
void func_dryfield_night_motel_lobby_8017F504(Task* task)
{
    ...
    s32                               key;
    s32                               one;
    DryfieldNightMotelLobbyCapScript* script;
    McSaveData*                       save;
    ...
    switch (task->state) {
        ...
        case 5:
            if (Pad_CheckButtons(0, 1, Pad_MaskConfirm | Pad_MaskCancel) != 0) {
                ...
                Task_Kill(D_dryfield_night_motel_lobby_801844D0);
                task->state++;
