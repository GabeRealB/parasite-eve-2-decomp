...
void func_shelter_b1_north_maintenance_walkway_8017EE48(GsCOORDINATE2* arg0, s16 arg1, s32 arg2, u8* arg3);
void func_shelter_b1_north_maintenance_walkway_8017F26C(GsCOORDINATE2* coord, s16 frame, u8* rgb);
void func_shelter_b1_north_maintenance_walkway_8017FB44(GsCOORDINATE2* coord, s16 frame);
void func_shelter_b1_north_maintenance_walkway_801803E8(GsCOORDINATE2* coord, s16 frame, u8* rgb);
    ...
    if (flag != 0) {
        ...
        switch (arg0->state) {
            ...
            case 1:
                ...
                rgb[2] = rgb[2] >> 1;
                if (mem->age & 1) {
                    func_shelter_b1_north_maintenance_walkway_8017F26C(coord, mem->angle + 0x100, rgb);
                }
                func_shelter_b1_north_maintenance_walkway_8017EE48(coord, 0x300 - (u16)mem->angle * 2, 0x80, rgb);
                if (arg0->spawnArg1 == 0) {
                    mem->scale  = 0xFF;
                    arg0->state = 2;
                    return;
...
void func_shelter_b1_north_maintenance_walkway_8017F998(Task* arg0)
{
    ...
    if (flag != 0) {
        ...
        step       = mem->angle + 0x10;
        mem->angle = step;
        func_shelter_b1_north_maintenance_walkway_8017F26C(coord, step * 2, rgb);
        func_shelter_b1_north_maintenance_walkway_8017FB44(coord, mem->angle);
        if (mem->period >= 0x19) {
            rgb[0] = mem->period;
            rgb[1] = (u16)mem->period >> 1;
            rgb[2] = (u16)mem->period >> 2;
            func_shelter_b1_north_maintenance_walkway_8017EE48(coord, mem->step * 3 / 2, 0x60, rgb);
            mem->period -= 0x18;
            mem->step   += 0x30;
            return;
        }
