...
/// Descriptor 2 of `D_dryfield_water_tower_8018277C` -- the last of the three
/// entries in the table whose entry 1 is `func_dryfield_water_tower_80180038`,
/// and the same body as that one run in the opposite direction. Its state 0
/// fills the 8-byte `DwtwFadeWork` block with 0xFF, i.e. white, instead of
/// clearing it, and every state-1 frame *drops* the three channels by
/// `Task::spawnArg1` where that task raises them: this is the fade the screen
/// comes back out of. `Fade_DrawOverlay` still takes red twice, for the green
/// argument as well as the red one, so only the green channel reads as a colour
/// here.
///
/// The red channel is the watched one in both directions, but the test flips
/// with the arithmetic and is signed: the task kills itself once the halfword
/// goes negative, where the other waits for it to pass 0x100. Both are the
/// same `s16` comparison on the `u16` field, which is why the two bodies differ
/// in nothing but the initial constant and the direction.
