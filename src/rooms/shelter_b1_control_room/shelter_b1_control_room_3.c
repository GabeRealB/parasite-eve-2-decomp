...
} _ShelterB1ControlRoomLight;

extern s32                        D_80070F70;
extern _ShelterB1ControlRoomLight D_80114FF8;
...
/// Effect task that moves the task object's coordinate toward the coordinate
/// in `spawnArg1`. On its first tick it takes their world displacement into
/// the frame of the coordinate's parent and scales it by 0xCC / 0x1000; after
/// that it adds that step every tick and draws a billboard there every other
    ...
    if (Gp_State1C->eventState == 0) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                Gp_UpdateCoord(coord);
                if (work->field_22 & 1) {
                    func_shelter_b1_control_room_801806FC(coord, (s16)++work->field_20, 0x200, 0x80);
                }
...
/// Draws a billboard at the screen position of the coordinate's world
...
/// event state is non-zero and released once it reaches 4. Every tick it
/// draws a fan of growing radius and the glow of
...
void func_shelter_b1_control_room_80181138(Task* arg0)
{
    ...
    if (flag != 0) {
        ...
        step       = mem->angle + 0x10;
        mem->angle = step;
        func_shelter_b1_control_room_80180DA4(coord, (s16)(step * 2), rgb);
        func_shelter_b1_control_room_801812E4(coord, mem->angle);
        if (mem->period >= 0x19) {
            rgb[0] = mem->period;
            rgb[1] = (u16)mem->period >> 1;
            rgb[2] = (u16)mem->period >> 2;
            func_shelter_b1_control_room_80180980(coord, (s16)(mem->step * 3 / 2), 0x60, rgb);
            mem->period -= 0x18;
            mem->step   += 0x30;
            return;
        }
...
/// scaled by 0x37 over the depth, and the flat quad of
