    ...
    if (Gp_State1C->eventState != 0) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                if (work->age & 1) {
                    work->index++;
                    func_dryfield_motel_balcony_8017DF84(coord, work->index, work->angle | 0x1000, work->scale | work->period);
                }
                if (work->scale > 0) {
            ...
            case 2:
                ...
                if (work->age & 1) {
                    work->index++;
                    func_dryfield_motel_balcony_8017DF84(coord, work->index, work->angle, work->scale | work->period);
                }
                if (work->scale > 0) {
                    if (work->step - 8 < work->age) {
                        work->scale -= 0x10;
...
/// the GTE flags the projection, queues sixteen gouraud `POLY_G4` wedges
...
void func_dryfield_motel_balcony_8017ED98(Task* arg0)
{
    ...
    if (flag != 0) {
        ...
        if (mem->period >= 0x19) {
            ...
            rgb[1] = mem->period >> 1;
            rgb[2] = mem->period >> 2;
            func_dryfield_motel_balcony_8017E248(coord, (s16)(mem->step * 3 / 2), 0x60, rgb);
            mem->period -= 0x18;
            mem->step   += 0x30;
            return;
        }
...
/// (each scaled by 0x37 / otz), plus the flat quad at 0x8017F470 on the
...
/// the effect at the first anchor and fills two eight-slot trails, one per
/// anchor, with the anchors' view-space positions. State 1 writes each
    ...
    if (Gp_State1C->eventState < 2) {
        ...
        switch (task->state) {
            case 0:
                ...
                objCoord->coord.t[0] = D_dryfield_motel_balcony_801822C0[0].vx;
                objCoord->coord.t[1] = D_dryfield_motel_balcony_801822C0[0].vy;
                objCoord->coord.t[2] = D_dryfield_motel_balcony_801822C0[0].vz;
                objCoord->flg        = 0;
                Gp_UpdateCoord(objCoord);
                task->state      = 1;
                coord.sub        = work->parent;
                vec              = &D_dryfield_motel_balcony_801822C0[1];
...
/// quads, walking backwards from slot `arg2`; each quad spans two adjacent
/// slots of `arg0` and `arg1`. The leading edge's brightness is
...
/// spark effect (state 1, which sprays randomly-moving sparks for seven
...
void func_dryfield_motel_balcony_80181628(Task* task)
{
    ...
    switch (task->state) {
        ...
        case 2:
            ...
            rgb[1]       = work->angle >> 1;
            rgb[2]       = work->angle >> 2;
            func_dryfield_motel_balcony_80180580(objCoord, 0x100, 0x100, rgb);
            func_dryfield_motel_balcony_80180580(objCoord, work->scale, work->scale, rgb);
            if (work->age >= 7) {
                task->state = 3;
            }
            break;
