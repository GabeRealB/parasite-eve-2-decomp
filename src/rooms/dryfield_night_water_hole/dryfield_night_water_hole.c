...
/// 0x7DB payload, picked by `gGameSession::field_8`. They are the last two of
...
///
/// On the visit whose sub-id (`gGameSession::field_9`) is 1 and that has
...
/// `gGameSession::field_8` selects. On sub-id 0xA, with pointer slot 0xA
...
void func_dryfield_night_water_hole_8017D958(Task* arg0)
{
    ...
    if (gGameSession->loc.place == 1 && Gp_LookupSlot4(0) != 0 && GameFlag_GetNibble(0x95) != 0) {
        if (gGameSession->loc.field_4 == 2) {
    ...
    if (gGameSession->loc.place == 0xA && Game_GetPtrSlot(0xA) != 0 && GameFlag_GetNibble(0xCF) == 0) {
