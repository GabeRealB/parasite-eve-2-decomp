...
/// 0x7DB payload, picked by `Game_Session::field_8`. They are the last two of
...
///
/// On the visit whose sub-id (`Game_Session::field_9`) is 1 and that has
...
/// `Game_Session::field_8` selects. On sub-id 0xA, with pointer slot 0xA
...
void func_dryfield_night_water_hole_8017D958(Task* arg0)
{
    ...
    if (Game_Session->field_9 == 1 && Gp_LookupSlot4(0) != 0 && GameFlag_GetNibble(0x95) != 0) {
    ...
    if (Game_Session->field_9 == 0xA && Game_GetPtrSlot(0xA) != 0 && GameFlag_GetNibble(0xCF) == 0) {
