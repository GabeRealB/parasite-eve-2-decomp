    ...
    if (Gp_State1C->eventState != 0) {
        if (Gp_State1C->eventState < 4) {
            if (task->state < 2) {
                ...
                func_neo_ark_bridge_8017FCA0(coord, work->index, work->scale);
            }
            return;
        }
        Gp_ReleaseState1CMem(work, task);
    ...
    if (Gp_State1C->eventState != 0) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                rgb[1] = work->scale >> 2;
                rgb[2] = work->scale >> 1;
                func_neo_ark_bridge_80180654(coord, work->angle, rgb);
                rgb[0] >>= 1;
                rgb[1] >>= 1;
                rgb[2] >>= 1;
            ...
            case 2:
                if (work->scale >= 0x11) {
                    ...
                    rgb[1] = work->scale >> 2;
                    rgb[2] = work->scale >> 1;
                    func_neo_ark_bridge_80181558(coord, (s16)(work->angle * 3), rgb);
                    work->scale -= 0x10;
                    work->angle -= 8;
                    break;
                }
    ...
    if (Gp_State1C->eventState < 2) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                Gp_UpdateCoord(objCoord);
                coord.sub        = work->parent;
                coord.coord.t[0] = D_neo_ark_bridge_80181F78.vx;
                coord.coord.t[1] = D_neo_ark_bridge_80181F78.vy;
                coord.coord.t[2] = D_neo_ark_bridge_80181F78.vz;
                coord.flg        = 0;
                Gp_UpdateCoord(&coord);
...
void func_neo_ark_bridge_801812D0(Task* task)
{
    ...
    switch (task->state) {
        ...
        case 2:
            ...
            rgb[1]       = work->angle >> 1;
            rgb[2]       = work->angle >> 2;
            func_neo_ark_bridge_80180228(objCoord, 0x100, 0x100, rgb);
            func_neo_ark_bridge_80180228(objCoord, work->scale, work->scale, rgb);
            if (work->age >= 7) {
                task->state = 3;
            }
            break;
