...
/// the island and the pavilion. A close relative of `RoomsShared8017f4a0`; the
/// only difference is that this one does not call `Gp_UpdateCoord`, so the
...
/// parameter, and once the ramp falls under 2 it releases the effect. After the
/// fade (`field_4 >= 4`) it only draws and releases, so the effect lingers
/// through the rest of the fade without advancing.
void RoomsShared8017eb68(Task* task)
{
    RoomEffWork*   work;
    GsCOORDINATE2* coord;
