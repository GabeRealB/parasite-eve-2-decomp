    ...
    if (Gp_State1C->eventState != 0) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                func_dryfield_night_junk_yard_8017EC98(coord, (s16)work->angle, rgb);
                rgb[0] >>= 1;
                rgb[1] >>= 1;
                rgb[2] >>= 1;
                func_dryfield_night_junk_yard_8017EC98(coord, (s16)((u16)work->angle * 2), rgb);
                func_dryfield_night_junk_yard_8017E86C(coord, (s16)(0x300 - (u16)work->angle * 2), 0x80, rgb);
                if (task->spawnArg1 == 0) {
                    work->scale = 0xFF;
                    task->state = 2;
                    rgb[0]      = work->scale;
            ...
            case 2:
                if ((s16)work->scale >= 0x11) {
                    ...
                    rgb[1] = (u16)work->scale >> 2;
                    rgb[2] = (u16)work->scale >> 1;
                    func_dryfield_night_junk_yard_8017FB9C(coord, (s16)((s16)work->angle * 3), rgb);
                    work->scale -= 0x10;
                    work->angle -= 8;
                    break;
                }
...
/// `D_dryfield_night_junk_yard_80180774` gives from the parent and fills the
    ...
    if (Gp_State1C->eventState < 2) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                Gp_UpdateCoord(objCoord);
                coord.sub        = work->parent;
                coord.coord.t[0] = D_dryfield_night_junk_yard_8018077C.vx;
                coord.coord.t[1] = D_dryfield_night_junk_yard_8018077C.vy;
                coord.coord.t[2] = D_dryfield_night_junk_yard_8018077C.vz;
                coord.flg        = 0;
                Gp_UpdateCoord(&coord);
...
void func_dryfield_night_junk_yard_8017F914(Task* task)
{
    ...
    switch (task->state) {
        ...
        case 2:
            ...
            func_dryfield_night_junk_yard_8017E86C(objCoord, 0x100, 0x100, rgb);
            func_dryfield_night_junk_yard_8017E86C(objCoord, work->scale, work->scale, rgb);
            if (work->age >= 7) {
                task->state = 3;
            }
            break;
