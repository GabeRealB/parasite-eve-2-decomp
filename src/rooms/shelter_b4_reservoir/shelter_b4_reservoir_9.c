...
/// `Display_ClampField126`. Afterwards the context's mode ramps the strength up to its limit
/// (mode 0), back down to zero and on to mode 2 (mode 1), or ends the task and
/// passes 0 back (mode 2); the displacement is the ramp's share of the
/// context's peak. A non-zero tint flag shades the quads with the context's
