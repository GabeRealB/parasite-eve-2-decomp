...
/// Fade-out task: on its first tick it allocates the 8-byte `r`/`g`/`b`
/// block and seeds all three channels to 0xFF, then every frame it draws the
/// fade overlay and steps each channel down by `Task::spawnArg1`. `r` is the
/// one the end-of-fade test watches, so once it has gone negative the task
/// kills itself. The mirror image of the fade-in task above; the same body
/// runs in `dryfield_gas_station` and `dryfield_water_tower`.
