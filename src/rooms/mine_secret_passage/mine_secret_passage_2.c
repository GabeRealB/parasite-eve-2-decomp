...
/// `data.light` takes the glow's position and a randomised intensity.
typedef struct {
    s32 mode;
    union {
        GsCOORDINATE2 coord;
...
} _MineSecretPassageLight;

extern s32                     D_80070F70;
extern _MineSecretPassageLight D_80114FF8;
...
/// Frame callback of a burst. State 0 seeds the levels; every tick then draws
/// a `func_mine_secret_passage_8017F21C` disc at twice the growing angle and a
/// `func_mine_secret_passage_8017FAF4` glow at the angle itself, both tinted
/// orange by the main level. While the echo level is above 0x18 it also draws
/// a widening `func_mine_secret_passage_8017EDF8` ring tinted by that level,
...
void func_mine_secret_passage_8017F948(Task* arg0)
{
    ...
    if (flag != 0) {
        ...
        step       = mem->angle + 0x10;
        mem->angle = step;
        func_mine_secret_passage_8017F21C(coord, (s16)(step * 2), rgb);
        func_mine_secret_passage_8017FAF4(coord, mem->angle);
        if (mem->period >= 0x19) {
            rgb[0] = mem->period;
            rgb[1] = (u16)mem->period >> 1;
            rgb[2] = (u16)mem->period >> 2;
            func_mine_secret_passage_8017EDF8(coord, (s16)(mem->step * 3 / 2), 0x60, rgb);
            mem->period -= 0x18;
            mem->step   += 0x30;
            return;
        }
...
/// Draws a glow at the coordinate: two camera-facing textured squares (tpage
...
/// outer one's CLUT. It also switches the gameplay light slot on at the
