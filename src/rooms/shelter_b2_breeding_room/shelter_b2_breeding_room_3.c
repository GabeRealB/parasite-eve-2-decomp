...
} _ShelterB2BreedingRoomLight;

extern s32                         D_80070F70;
extern _ShelterB2BreedingRoomLight D_80114FF8;
...
void func_shelter_b2_breeding_room_8017E774(Task* arg0)
{
    ...
    switch (arg0->state) {
        ...
        case 2:
            ...
            col[2] >>= 1;
            if (mem->age & 1) {
                func_shelter_b2_breeding_room_8017F598(coord, (s16)(mem->angle + 0x100), col);
            }
            break;
        case 3:
            Gp_UpdateCoord(coord);
            ...
            func_shelter_b2_breeding_room_8017F174(coord, (s16)(mem->period + 0x80), 0x100, col);
            mem->angle -= 0x10;
            if (mem->scale > 0x10) {
                mem->scale -= 0x10;
                break;
    ...
    if (Gp_State1C->eventState == 0) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                Gp_UpdateCoord(coord);
                if (work->field_22 & 1) {
                    func_shelter_b2_breeding_room_8017EEF0(coord, (s16)++work->field_20, 0x200, 0x80);
                }
...
void func_shelter_b2_breeding_room_8017F92C(Task* arg0)
{
    ...
    if (flag != 0) {
        ...
        step       = mem->angle + 0x10;
        mem->angle = step;
        func_shelter_b2_breeding_room_8017F598(coord, (s16)(step * 2), rgb);
        func_shelter_b2_breeding_room_8017FAD8(coord, mem->angle);
        if (mem->period >= 0x19) {
            rgb[0] = mem->period;
            rgb[1] = (u16)mem->period >> 1;
            rgb[2] = (u16)mem->period >> 2;
            func_shelter_b2_breeding_room_8017F174(coord, (s16)(mem->step * 3 / 2), 0x60, rgb);
            mem->period -= 0x18;
            mem->step   += 0x30;
            return;
        }
