...
/// One frame of a promenade glow sprite: a camera-facing, semi-transparent
...
///
/// Like the ground glow above, the task is one-shot: the work block is released
...
void func_acropolis_promenade_8017F0BC(Task* task)
{
    ...
    register ApmGlowSpriteScratch* p asm("a0");
    POLY_FT4*                      prim;
    s32                            grey;
    s32                            clut;
    ...
    scratch     = (void**)G_SCRATCH_HEAD;
    head        = *scratch;
    blk         = (ApmGlowSpriteScratch*)(head - 0x14);
    blk->pos.vx = coord->workm.t[0];
    blk->pos.vy = coord->workm.t[1];
    *scratch    = blk;
    p           = blk;
