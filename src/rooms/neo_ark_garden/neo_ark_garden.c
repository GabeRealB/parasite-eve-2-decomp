...
/// Scratchpad block the water ripple borrows from `G_SCRATCH_HEAD` for one
/// call: the transposed view rotation, the camera-space row vector fed to the
/// GTE (`row`) and its rotated result (`rowView`), the rotated view
/// translation (`origin`) and the numerator of the per-row depth division
/// (`depth`). The block is 0x4C bytes; nothing reads the tail.
typedef struct {
    MATRIX  mtx;
    SVECTOR row;
    SVECTOR rowView;
...
static inline void _neoArkGardenRotTrans(MATRIX* m, SVECTOR* v)
{
    SVECTOR tmp;
...
void func_neo_ark_garden_8017D64C(Task* task)
{
    ...
    s32                         area;
    POLY_FT4*                   prim;
    _NeoArkGardenRippleScratch* scratch;
    s32                         y;
    s32                         y0;
    s32                         xl;
    s32                         xr;
    ...
    *(_NeoArkGardenRippleScratch**)G_SCRATCH_HEAD = *(_NeoArkGardenRippleScratch**)G_SCRATCH_HEAD - 1;
    scratch                                       = *(_NeoArkGardenRippleScratch**)G_SCRATCH_HEAD;
    ...
    _neoArkGardenRotTrans(&scratch->mtx, &scratch->origin);
    ...
    *(_NeoArkGardenRippleScratch**)G_SCRATCH_HEAD += 1;
}
...
/// start and end rows gets one or two `POLY_FT4` strips that sample the other
