...
extern SVECTOR D_shelter_b4_lower_sewer_80181F94[];
...
void func_shelter_b4_lower_sewer_8017EEE4(Task* task)
{
    ...
    if (Gp_State1C->eventState != 0) {
        func_shelter_b4_lower_sewer_8017F038(coord, (s16)work->field_26, (s16)work->field_24);
...
/// number of ticks per animation frame and, when the spawner left `field_10`
/// zero, a velocity chosen by bits 24-27 of `spawnArg1`, normalised and scaled
/// by `field_2A` through the GTE. Later ticks draw, drift the coordinate by
/// that velocity with `vy` growing by 6 each tick, and advance the animation
/// frame every `field_28` ticks, releasing the task after frame 7. While an
...
void func_shelter_b4_lower_sewer_8017F36C(Task* task)
{
    ...
    if (Gp_State1C->eventState != 0) {
        if (Gp_State1C->eventState < 4) {
            if (task->state < 2) {
                func_shelter_b4_lower_sewer_8017F828(coord, work->field_20, (s16)work->field_24, (s16)work->field_26);
            } else {
                func_shelter_b4_lower_sewer_8017FC14(coord, work->field_20, (s16)work->field_24);
            }
            return;
        }
        Gp_ReleaseState1CMem(work, task);
    ...
    if (Gp_State1C->eventState != 0) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                func_shelter_b4_lower_sewer_80180580(coord, (s16)work->angle, rgb);
                rgb[0] >>= 1;
                rgb[1] >>= 1;
                rgb[2] >>= 1;
            ...
            case 2:
                if ((s16)work->scale >= 0x11) {
                    ...
                    rgb[1] = (u16)work->scale >> 2;
                    rgb[2] = (u16)work->scale >> 1;
                    func_shelter_b4_lower_sewer_80181484(coord, (s16)((s16)work->angle * 3), rgb);
                    work->scale -= 0x10;
                    work->angle -= 8;
                    break;
                }
    ...
    if (Gp_State1C->eventState < 2) {
        ...
        switch (task->state) {
            ...
            case 1:
                ...
                Gp_UpdateCoord(objCoord);
                coord.sub        = work->parent;
                coord.coord.t[0] = D_shelter_b4_lower_sewer_80181F9C.vx;
                coord.coord.t[1] = D_shelter_b4_lower_sewer_80181F9C.vy;
                coord.coord.t[2] = D_shelter_b4_lower_sewer_80181F9C.vz;
                coord.flg        = 0;
                Gp_UpdateCoord(&coord);
