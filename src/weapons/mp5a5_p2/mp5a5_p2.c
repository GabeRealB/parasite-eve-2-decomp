...
void func_mp5a5_p2_8011D468(GsCOORDINATE2* arg0, s16 arg1, s16 arg2);
void func_mp5a5_p2_8011D864(GsCOORDINATE2* arg0, s16 arg1, s16 arg2);

extern s32 Gp_LcgState;

/// Muzzle offset of the MP5A5(+2), in the firing hand's coordinate frame.
SVECTOR D_mp5a5_p2_8011E128 = { 0, 0x240, 0x40, 0 };
...
void func_mp5a5_p2_8011D1E0(Task* task)
{
    GpEffWork*     work;
    GsCOORDINATE2* coord;
    GpCoord64*     base;
    ...
    switch (task->state) {
        case 0:
            ...
            coord->coord.t[0] = D_mp5a5_p2_8011E128.vx;
            coord->coord.t[1] = D_mp5a5_p2_8011E128.vy;
            coord->coord.t[2] = D_mp5a5_p2_8011E128.vz;
            coord->flg        = 0;
            ...
            for (i = 0; i < 4; i++) {
                Gp_LcgState            = Gp_LcgState * 5 + 0x71357911;
                D_mp5a5_p2_8012B508[i] = ((i & 3) << 10) + (((u32)Gp_LcgState >> 16) & 0x3FF);
            }
            break;
        case 1:
    ...
    func_mp5a5_p2_8011D468(coord, work->scale, work->angle);
    ...
    for (i = 0; i < 4; i++) {
        func_mp5a5_p2_8011D864(coord, D_mp5a5_p2_8012B508[i], work->period);
    }
...
/// Per-frame firing state machine for the MP5A5(+2). State 0 arms the shot and
...
/// single fire (`== 1`) spends one round, plays `0x20200004`, spawns the plain
/// muzzle flash and runs the recoil clip, while burst fire spends 0x101, plays
/// `0x20200005`, holds the pose for 0x12 frames and reparents the longer flash
...
void func_mp5a5_p2_8011DDA4(GpActorWork* arg0)
{
    GameActor*     actor;
    GsCOORDINATE2* coord;
    GsCOORDINATE2* spot;
        ...
        case 3:
            if (--actor->field_934 == 0) {
                ...
                if (actor->field_97F == 1) {
                    ...
                    Gp_PlayObjSfx(arg0->extra->coords, 0x20200004, 1);
                    Gp_SpawnEff(0x6002B,
                                (GsCOORDINATE2*)((TmdObject*)actor->field_91C->extra)->coords,
                                0x20, NULL);
                    Gp_ConsumeSlotQty(0x9F, 1);
                    ...
                    Gp_PlayObjSfx(arg0->extra->coords, 0x20200005, 0);
                    Gp_ConsumeSlotQty(0x9F, 0x101);
