...
void func_mm1_8011DB94(Task* task);
...
/// places the projectile at the per-ammo muzzle offset from `D_mm1_8012D2C0`, parents it
/// to the world coordinate, sets its launch speed from `D_mm1_8012D2D8` and links its two
/// collision nodes. MM1 and Grenade Pistol carry identical copies.
void func_mm1_8011D3A8(Task* arg0)
{
    void**           scratch;
    u8*              head;
    SVECTOR*         blk;
    ...
    *scratch = blk;
    ...
    arg0->exitCallback = func_mm1_8011DB94;
    arg0->state++;
    Mem_Set(work, 0, sizeof(M4a1GrenadeWork));
    blk->vx     = D_mm1_8012D2C0[idx].vx;
    blk->vy     = D_mm1_8012D2C0[idx].vy;
    blk->vz     = D_mm1_8012D2C0[idx].vz;
    muzzle->flg = 0;
...
/// Flight state of the MM1's projectile. Detonates when the shot
...
void func_mm1_8011D704(Task* arg0)
{
    ...
    *(void**)G_SCRATCH_HEAD = head - sizeof(Mm1Scratch);
    blk                     = (Mm1Scratch*)(head - sizeof(Mm1Scratch));
    coord->flg              = 0;
...
/// identical copies.
void func_mm1_8011DB68(Task* task)
{
...
{
    Mm1StateFn states[4] = {
        (Mm1StateFn)func_mm1_8011D3A8,
        (Mm1StateFn)func_mm1_8011D704,
        (Mm1StateFn)func_mm1_8011DB68,
        (Mm1StateFn)func_mm1_8011DB94,
    };
