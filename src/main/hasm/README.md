...
| `tmdSkipStreamRecord.s` | `tmdSkipStreamRecord` | `0x800105AC` | Handler of a record with no other handler: steps over its elements |
...
| `Tmd_StreamHandlers_Ops.s` | `Tmd_StreamHandler_Op20` … `Op130`, `tmdStreamPrimGt4CornerColors` (20 funcs) | `0x80010A90`–`0x80012750` | Early-image draw-pass stream handlers: one per opcode, named for the command it serves where that has been read |
