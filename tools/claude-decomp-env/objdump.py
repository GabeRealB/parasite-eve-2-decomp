...
RE_SOURCE_LINE = re.compile(
    r"^[a-zA-Z0-9_/\.\-]+\.[ch]:\d+"
)  # Matches "src/temp.c:336"
RE_ASM_LINE = re.compile(
    r"^\s*([0-9a-f]+):\s+([0-9a-f]+)\s+(.+)$"
)  # Matches " 794:    2402001e     li    v0,30"
RE_RELOC_LINE = re.compile(
    r"^\s+([0-9a-f]+):\s+(R_MIPS_\S+)\s+(.+)$"
)  # Matches "            7a0: R_MIPS_26    setGameStateHandler"

# MIPS objdump settings
OBJDUMP_EXECUTABLES = [
    "mips-linux-gnu-objdump",
]
OBJDUMP_ARGS = ["-drz", "-m", "mips:3000"]
...
def normalize_jumptable_references(line: str) -> str:
    ...
    # Jump table references: jtbl_80013EB0 in main/gameplay, but
    # jtbl_actor_203700_80149E34 in the overlays, which are the large majority.
    # Match the whole symbol - a hex-only pattern stops at the first non-hex
    # character and leaves spliced garbage like ".rodatator_203700_80149E34",
    # because "ac" of "actor" is itself valid hex.
...
def process_annotated_dump_lines(
    lines: List[str], include_source: bool = True
) -> List[str]:
    ...
    while i < len(lines):
        ...
        if (
            line.startswith("        ")
            and not RE_ASM_LINE.match(line)
            and not RE_RELOC_LINE.match(line)
        ):
        ...
        if asm_match:
            ...
            formatted = (
                f"{mnemonic}\t{args}".replace("\t", " " * 4) if args else mnemonic
            )
    ...
    try:
        if args.from_dump:
            ...
            with open(args.file, "r") as f:
                lines = f.readlines()
            output_lines = process_annotated_dump_lines(
                lines, include_source=not args.no_source
            )
        else:
