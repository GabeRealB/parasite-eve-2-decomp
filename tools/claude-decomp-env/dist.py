...
def check_assertions(
    target_lines: List[str], base_lines: List[str], assertions: List[str]
) -> bool:
...
def score_files(
    ...
    normalized_target_lines = [
        normalize_jumptable_references(line) for line in target_lines
    ]
    normalized_cand_lines = [
        normalize_jumptable_references(line) for line in cand_lines
    ]
    ...
    if debug_mode:
        ...
        print(
            f" Lines Matched: {total_equal_lines}/{total_lines} ({match_percentage:.1f}%)"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Score the difference between two assembly files."
    )
    ...
    parser.add_argument(
        "--stack-diffs",
        action="store_true",
        help="Preserve stack differences (for .o files)",
    )
    ...
    if args.target_file.endswith(".o") and args.cand_file.endswith(".o"):
        ...
        target_lines = read_object_file(
            args.target_file, preserve_offsets=args.stack_diffs
        )
