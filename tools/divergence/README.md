...
## Generate structured transformations automatically

```sh
python3 tools/divergence/toolset.py smart SCRATCH base_1 \
  --function FUNCTION --output /tmp/smart-search \
  --budget 64 --beam 4 --fanout 8 --depth 3 --seed 0
```

`smart` generates fresh AST transformations from each candidate; it does not
need a mutation plan. It reuses the scratch's ordinary `build.sh`, experiment
journal, independent scorer, and optional compiler-state constraints:

```sh
python3 tools/divergence/toolset.py smart SCRATCH base_1 \
  --output /tmp/branch-search --operators invert_if guard_else join_returns \
  --constraints /tmp/constraints.json --budget 32
```

The constraints file is a JSON list in the format described above. The saved
candidate `.i` is required. Parsing uses the bundled `decomp-permuter` GNU C
parser. Only the selected function body is replaced in the original `.c`;
surrounding headers, declarations and other functions retain their original
text. Body macros are expanded for the current compilation. Directives inside
the body, location-dependent macros, unsupported syntax and ambiguous function
selection are rejected explicitly. Supply `--function` for multiple functions.

| Operator | Generated experiment |
|---|---|
| `invert_if` | Negate the condition once and exchange braced arms; supply an empty arm when needed |
| `guard_else` | Move the false arm after a true arm ending in return, preserving scope |
| `join_returns` | Put a following return in an explicit else arm |
| `temporary` | Assign through a fresh temporary with the destination's integer type |
| `dead_store` | Initialize that temporary to zero, then overwrite it before use |
| `sched_barrier` | Insert empty volatile GNU asm before a statement |
| `memory_barrier` | Insert empty volatile GNU asm with a memory clobber |
| `volatile_temp` | Materialize through a fresh volatile integer temporary; select explicitly with `--operators` |

All except `volatile_temp` are enabled by default. Typed rewrites exclude
ambiguous/shadowed declarations, qualified destinations, pointers and static or
external storage. Local typedefs disable typed rewrites conservatively.
Control-flow edits retain condition evaluation and arm contents; label/goto
cases that need additional analysis are excluded from `guard_else`.
Volatile temporaries add accesses and are recorded as access-changing
experiments. Barriers are compiler-specific dependency experiments. These
classifications and the recorded prerequisites are not a semantic proof.

The build budget includes an unchanged baseline rebuild, a body printing
control, and failures. A changed baseline score stops the search. The printing
control measures whether macro expansion/printing alone changes output, and
its result is reported separately. Subsequent generations prioritize operators
using current mismatch families, supported input-line locations and measured
per-operator success counts. Operators and beam parents share each generation's
budget. `--fanout` bounds new candidates per parent; `--depth` bounds rewrite
chains. A seeded exploration slot in beams of at least three permits neutral
or worse intermediate results. Different sources with identical assembly are
retained; previously visited identical sources are skipped.

`search.json` records termination, baseline/control/best scores, constraints,
generation errors and operator feedback. `manifest.json` records each build,
parent, transformation prerequisites and complete rewrite history. `best.c`
contains the lowest-distance candidate; the original remains eligible.
Constraints remain explicit; `best_objectives_candidate` identifies
the best objective-ranked alternative. No winner is promoted into the project.
Score zero with all supplied constraints met stops the search, but still needs
normal integration verification. Budget/depth exhaustion is a completed search,
not a match. Unsupported input or failed controls exit with status 2.

Reproduce the structured real-compiler tests with a new output directory:

```sh
python3 -B tools/divergence/tests/smart_smoke.py --output /tmp/smart-checks
```

The bundled cases test branch inversion, barrier placement, temporary
materialization and a combination of two transformations. They contain explicit
target and baseline sources, require no hand-authored mutation plans, and
independently rebuild and byte-check winning `.text` sections. They are selected
synthetic cases, not a benchmark establishing an advantage over another permuter.
