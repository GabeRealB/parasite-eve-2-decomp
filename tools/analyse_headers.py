...
# Data structures
FunctionDef = namedtuple(
    "FunctionDef", ["name", "file", "line", "signature", "is_static"]
)
...
class CProjectAnalyzer:
    def __init__(self, project_dir):
        ...
        self.c_file_declarations = defaultdict(
            list
        )  # func_name -> [FunctionDecl] (declarations in .c files)

        # Regex patterns
        self.func_def_pattern = re.compile(
            r"^(?!.*static\s+)(\w+(?:\s*\*)*)\s+(\w+)\s*\([^)]*\)\s*\{", re.MULTILINE
        )
        self.static_func_pattern = re.compile(
            r"^static\s+\w+(?:\s*\*)*\s+(\w+)\s*\([^)]*\)\s*\{", re.MULTILINE
        )
        self.extern_pattern = re.compile(r"extern\s+([^;]+;)", re.MULTILINE)
        ...
        self.include_asm_pattern = re.compile(
            r"INCLUDE_ASM\s*\([^,]+,\s*(\w+)\s*\)", re.MULTILINE
        )
    ...
    def analyze_c_file(self, filepath):
        ...
        for match in self.extern_pattern.finditer(content):
            ...
            if func_name:
                line_num = content[: match.start()].count("\n") + 1
                self.extern_declarations[func_name].append(
                    ExternDecl(
                        name=func_name,
                        file=str(filepath),
                        line=line_num,
        ...
        for match in self.c_func_decl_pattern.finditer(content):
            ...
            if func_name and func_name not in [
                ...
                line_num = content[: match.start()].count("\n") + 1
                self.c_file_declarations[func_name].append(
                    FunctionDecl(
                        name=func_name,
                        file=str(filepath),
                        line=line_num,
    ...
    def generate_report(self):
            ...
            if func_name in self.function_definitions:
                ...
                if not has_header_decl:
                    ...
                    if not (len(extern_files) == 1 and definition.file in extern_files):
                        ...
                        suggested_header = self.suggest_header_file(
                            func_name, definition.file
                        )
                        issues.append(
                            {
                    ...
                    header_files = [
                        decl.file for decl in self.header_declarations[func_name]
                    ]
                    for extern_decl in extern_decls:
                        extern_file_includes = self.file_includes.get(
                            extern_decl.file, set()
                        )

                        # Check if any of the headers containing this function are included
                        header_included = any(
                            os.path.basename(hf) in extern_file_includes
                            or hf in extern_file_includes
        ...
        print(
            f"Extern declarations found: {sum(len(decls) for decls in self.extern_declarations.values())}"
        )
        print(
            f"Header declarations found: {sum(len(decls) for decls in self.header_declarations.values())}"
        )
        ...
        if issues:
            ...
            for i, issue in enumerate(issues, 1):
                ...
                if issue["type"] == "missing_header_declaration":
                    ...
                    print(
                        f"   RECOMMENDATION: Add declaration to {issue['suggested_header']}"
                    )
                    if "INCLUDE_ASM" in definition.signature:
                        ...
                        print(
                            f"   Add this line: {definition.signature.replace('{', ';')}"
                        )
                    ...
                    print(
                        f"   Available in headers: {', '.join(issue['available_headers'])}"
                    )
                    ...
                    print(
                        f"   RECOMMENDATION: Move declaration to appropriate header file"
                    )
