...
PACKAGE_ID_SRC = """\
/* GENERATED by tools/gen_overlay_configs.py from configs/USA/overlays.toml.
 *
 * Every package opens with its own id, a u16 in a u32 slot. The value reaches
 * the compiler as PKG_ID, from the id the package declares in the manifest, so
 * it is compiled in from the declaration rather than split out of the package:
 * the checksum then tests the declared value, and a manifest naming the wrong
 * id fails the build instead of reproducing whatever the package already said.
 *
 * Passing it as a flag is what lets this unit be identical for every package,
 * including the several load slots one image is issued at: they share every
 * other unit and differ only here.
 *
 * Nothing refers to the word. It is program structure written by the
 * packaging tool, so the unit exists to place the bytes, not to be referenced -
 * which is why it is private.
 *
 * It is `const` because the id belongs to the image's
...
def package_id_unit(family: str, package: str) -> str:
    """The unit holding a package's id word.

    One unit name for every overlay, under the package's own directory, so an
    entry covering several load slots names the same unit for each of them and
    still gets that slot's own id: the build keys an object on its source path,
    so a per-package path is what makes a per-package value possible at all.
    """
    return f"{package}/packageid"


def write_package_id(family: str, package: str) -> None:
    src = Path(f"src/{family}/{package}/packageid.c")
    src.parent.mkdir(parents=True, exist_ok=True)
    text = PACKAGE_ID_SRC
    if not src.is_file() or src.read_text(encoding="utf-8") != text:
...
def object_subsegments(
    ...
    An object is one translation unit with runs in whichever sections it owns,
    or a run of assembly nobody has claimed. splat lists an object in the linker
    script at its first subsegment and concatenates a section's contributions in
    that order, so the single ordered list the manifest carries is exactly what
    determines link order - there is nothing left to stitch together here.

    Assembly runs are named for the offset they start at. Naming them by their
    ...
    for obj in objects:
        ...
        if kind == "packageId":
            if pkg_id is None:
                raise SystemExit(
                    f"{name}: has a packageId object but declares no `id`")
...
def slot_packages(spec: dict) -> list[tuple[str, dict, dict]]:
    """(package, slot record, entry) for every package a family declares.

    An entry covers one package, or - where a family loads the same image into
    several slots - all of them at once. Expanding here keeps everything below
    working per package, which is what a config describes.
    """
    out = []
...
def generate(family: str, spec: dict, template: str, out_dir: Path) -> list[Path]:
    ...
    for name, slot, entry in slot_packages(spec):
        ...
        declared = slot.get("text", entry.get("text"))
        span = tuple(int(str(v), 16) for v in declared) if declared else text_span(data)
