...
def pack_context(func_name: str, version: Optional[str] = None) -> str:
    ...
    lines += [
        ...
        "`register … asm(\"\")` pins.",
        "From project root after a 100% match, verify with",
        "`./tools/build-and-verify.sh` and require `✅ BUILD SUCCEEDED`.",
        "It re-splits and rebuilds only what changed, so it is already the fast",
        "check; `--only <scope>` is only worth it while iterating on a header",
        "that many overlays include, and it says nothing about the rest.",
        ...
        "<overlay>`. A generated jump table must own *and start* its unit's .rodata.",
